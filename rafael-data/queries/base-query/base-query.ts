...
export default class BaseQuery {
  ...
  async create(
    ...
    {
      onConflictQry = null,
      onConflictUpdateAll = false,
      onConflictNothing = false,
    } = {}
  ) {
    if (Object.keys(data).length <= 0) return null;

    const columnNames = Object.keys(data).map((colName) =>
      this.automaticTransformColumn ? snakeCase(colName) : colName
    );
    const sqlParams =
      Object.keys(data).map((columnName) => data[columnName]) || [];
    const bindingValues = Array.from(
      Array(sqlParams.length),
      (_, i) => `$${i + 1}`
    );
    ...
    } else if (onConflictNothing && this.uniqueColumns.length > 0) {
      sql += `ON CONFLICT ("${this.uniqueColumns.join('", "')}") DO NOTHING `;
    } else if (onConflictUpdateAll && this.uniqueColumns.length > 0) {
      let updates: string[] = [];
      for (let columnName of columnNames) {
        if (
          !this.uniqueColumns.includes(columnName) &&
          columnName !== this.createdAtColumn
        ) {
          updates.push(`"${columnName}" = EXCLUDED."${columnName}"`);
        }
      }
      sql += `ON CONFLICT ("${this.uniqueColumns.join(
        '", "'
  ...
  buildWhereQryAndParams(where = {}, whereQry = [], sqlParams = []) {
    Object.keys(where).map((key) => {
      const columnName = this.automaticTransformColumn ? snakeCase(key) : key;
      const isPresent = Array.isArray(where[key])
        ? where[key].length > 0
        : !!where[key];
      if (isPresent) {
        if (Array.isArray(where[key])) {
          whereQry.push(
            `"${columnName}" IN ('${where[key]
              .map((val) => String(val).split(`'`).join(`''`))
              .join(`', '`)}')`
          );
        } else {
          const operator = this.caseInsensitiveColumns.includes(columnName)
            ? "ILIKE"
            : "=";
          if (this.isSqlSyntax(where[key])) {
            ...
            whereQry.push(
              `"${columnName}" ${operator} $${sqlParams.length + 1}`
            );
